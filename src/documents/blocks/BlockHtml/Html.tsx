...
export function Html({ style, props }: HtmlProps) {
  ...
  // const extractText = children?.split(/{{(.*?)}}/g);

  // const text = extractText?.map((item, index) => {
  //   if (index % 2 === 0) {
  //     return item;
  //   }
  //   return `<span class="notranslate" translate="no">{{${item}}}</span>`;
  // });
  ...
  // return <div style={cssStyle} dangerouslySetInnerHTML={{ __html: text?.join('') ?? '' }} />;
